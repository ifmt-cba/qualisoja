...
class AnaliseTeorOleoForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "peso_tara": forms.NumberInput(
                attrs={
                    "step": "0.001",
                    "placeholder": "Peso da tara (recipiente vazio)",
                }
            ),
            "peso_liquido": forms.NumberInput(
                attrs={"step": "0.001", "placeholder": "Peso líquido de óleo"}
            ),
    ...
    def clean(self):
        ...
        if peso_amostra is not None:
            if not (2.000 <= float(peso_amostra) <= 2.500):
                self.add_error(
                    "peso_amostra",
                    "O peso da amostra deve estar entre 2,000 g e 2,500 g.",
                )

        # Validar peso da tara
        if peso_tara is not None:
            if peso_tara <= 0:
                self.add_error("peso_tara", "O peso da tara deve ser positivo.")

        # Validar peso líquido
        if peso_liquido is not None:
            if peso_liquido <= 0:
                self.add_error("peso_liquido", "O peso líquido deve ser positivo.")
            if peso_tara is not None and peso_liquido < peso_tara:
                self.add_error(
                    "peso_liquido",
                    "O peso líquido (tara + óleo) deve ser maior que o peso da tara vazia.",
                )

        # Validar teor de óleo (se fornecido manualmente)
        if teor_oleo is not None and (teor_oleo < 0 or teor_oleo > 30):
            self.add_error("teor_oleo", "O teor de óleo deve estar entre 0% e 30%.")
...
class AnaliseFibraForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "horario": forms.TimeInput(attrs={"type": "time"}),
            "peso_amostra": forms.NumberInput(
                attrs={"step": "0.0001", "placeholder": "Peso da amostra"}
            ),
            "peso_tara": forms.NumberInput(
                attrs={
                    "step": "0.0001",
            ...
            "peso_fibra": forms.NumberInput(
                attrs={"step": "0.0001", "placeholder": "Peso da fibra"}
            ),
            "peso_branco": forms.NumberInput(
                attrs={"step": "0.0001", "placeholder": "Peso do branco"}
            ),
        }
    ...
    def clean(self):
        ...
        if peso_fibra is not None and peso_branco is not None and peso_tara is not None:
            if (peso_fibra + peso_branco) > peso_tara:
                self.add_error(
                    "peso_fibra",
                    "A soma do peso da fibra e peso do branco não pode ser maior que o peso da tara.",
                )

        # Validar se o resultado calculado é razoável (entre 0% e 50%)
        if all([peso_amostra, peso_tara, peso_fibra, peso_branco]):
            resultado_calc = (
                (peso_tara - peso_fibra - peso_branco) / peso_amostra
            ) * 100
            if resultado_calc < 0:
                self.add_error(
                    "peso_tara",
                    "Os valores informados resultam em um teor de fibra negativo. Verifique os pesos.",
                )
            elif resultado_calc > 50:
                self.add_error(
                    "resultado",
                    "O teor de fibra calculado é muito alto (>50%). Verifique os valores.",
                )

        resultado = cleaned_data.get("resultado")
        if resultado is not None and (resultado < 0 or resultado > 50):
            self.add_error("resultado", "O teor de fibra deve estar entre 0% e 50%.")
...
class AnaliseFosforoForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "tipo_amostra": forms.Select(attrs={"class": "form-select"}),
            "absorbancia_amostra": forms.NumberInput(
                attrs={
            ...
            "casas_decimais": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "min": "0",
                    "max": "6",
            ...
            "concentracao_padrao": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "step": "0.0001",
                    "min": "0.0001",
            ...
            "volume_solucao": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "step": "0.01",
                    "min": "0.01",
            ...
            "volume_aliquota": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "step": "0.01",
                    "min": "0.01",
            ...
            "absorbancia_padrao": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "step": "0.000001",
                    "min": "0.000001",
    ...
    def clean(self):
        ...
        if not absorbancia_amostra or absorbancia_amostra <= 0:
            self.add_error(
                "absorbancia_amostra",
                "A absorbância da amostra é obrigatória e deve ser maior que zero.",
            )
...
class AnaliseSilicaForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "data": forms.DateInput(attrs={"type": "date"}),
            "horario": forms.TimeInput(attrs={"type": "time"}),
            "resultado_silica": forms.NumberInput(
                attrs={
                    "step": "0.01",
                    "placeholder": "Ex: 1.25",
                    "class": "form-control",
            ...
            "analise_cinza": forms.Select(attrs={"class": "form-select"}),
        }
    ...
    def clean(self):
        ...
        if resultado_silica and resultado_silica <= 0:
            self.add_error(
                "resultado_silica", "O resultado da sílica deve ser maior que zero."
            )
