...
class AnaliseCinza(BaseModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.peso_amostra and self.peso_cadinho and self.peso_cinza:
            self.resultado = (
                (self.peso_cinza - self.peso_cadinho) / self.peso_amostra
            ) * 100
        super().save(*args, **kwargs)
...
class AnaliseTeorOleo(BaseModel):
    ...
    def clean(self):
        ...
        if self.peso_tara and self.peso_tara <= 0:
            raise ValidationError(
                {"peso_tara": "O peso da tara deve ser maior que zero."}
            )
    ...
    def save(self, *args, **kwargs):
        ...
        """
        if (
            self.peso_tara is not None
            and self.peso_liquido is not None
            and self.peso_amostra is not None
            and self.peso_amostra != 0
        ):

            self.teor_oleo = round(
                ((self.peso_liquido - self.peso_tara) / self.peso_amostra)
                * Decimal("100"),
                2,
            )
        super().save(*args, **kwargs)
    ...
    def __str__(self):
        return (
            f"Análise de Teor de Óleo - {self.get_tipo_amostra_display()} - {self.data}"
        )
...
class AnaliseFibra(BaseModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            ...
            and self.peso_tara
            and self.peso_fibra
            and self.peso_branco
        ):
            # Fórmula: (peso_tara - peso_fibra - peso_branco) / peso_amostra * 100
            self.resultado = (
                (self.peso_tara - self.peso_fibra - self.peso_branco)
                / self.peso_amostra
            ) * 100
        super().save(*args, **kwargs)
        ...
        if self.absorbancia_amostra:
            try:
                ...
                if ap != 0 and val != 0 and p != 0:
                    ...
                    concentracao_original = concentracao_aliquota * (
                        v / val
                    )  # mg/L na solução original
...
class AnaliseSilica(BaseModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            self.analise_cinza
            and self.analise_cinza.resultado
            and self.resultado_silica
        ):
